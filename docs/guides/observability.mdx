...
### Reserved override keys (opt-in)

Set `AEGRA_TRACE_METADATA_OVERRIDES=true` to opt into user-wins semantics for a small reserved set of metadata keys. When enabled, these keys are extracted from the request `metadata` before merge and override the corresponding top-level OTEL attribute:

- `session_id` — overrides `langfuse.session.id` / `session.id` (default: `thread_id`)

This is useful for multi-agent topologies where a parent run's identifier should group all worker runs under the same Langfuse session, regardless of which thread each worker runs on. Without the flag (the default), `session_id` is treated as a regular sub-field metadata key and surfaces as `langfuse.trace.metadata.session_id`.

Override values are validated independently of the `RunCreate` field validator (non-string, non-printable, or > 256-char values are dropped with a warning), providing a defense-in-depth guard for paths that store metadata in JSONB and bypass the request-time validator.
...
| `AEGRA_TRACE_METADATA_OVERRIDES` | Opt-in user-wins for reserved metadata keys: `session_id` (default: `false`) |
